...
outliers.viz.map = function(options)
{
    ...
    self.init = function(){
            self.width = 1200,
            self.height = 700,
            self.selectedProperty = "population"
            ...
            self.ready = function(error, ine) {
                ...
                ine.forEach(function (d, i) {
                    if (i == 0) { self.fillSelector(Object.keys(d)); }
            ...
            self.fillSelector = function(keys) {
                ...
                self.render = function(){
                    self.svg.selectAll(".subunit")
                       .data(topojson.feature(self.es, self.es.objects.subunits).features)
                       .enter()
                       .append("path")
                       .attr("class", function (d) { return "subunit " + d.id.slice(0, 3); })
                       .on("click",function(d){self.callBackMapa(d);})
                       .attr("d", self.path);
                    self.svg.append("path")
                       .datum(topojson.mesh(self.es, self.es.objects.subunits, function (a, b) { return a !== b && a.id.slice(0, 3) === "ESP"; }))
                       .attr("d", self.path)
                    ...
                    self.svg.selectAll(".subunit-label")
                       .data(topojson.feature(self.es, self.es.objects.subunits).features)
                       .enter()
                       .append("text")
